# Giovanni *(Giovy)* 👋

> 🎓 **CS Student @ UniPi** | Web, Backend & Networking Developer

Sono uno studente di Informatica in procinto di laurearmi a Pisa. Mi appassiona il mondo dell'infrastruttura web, le architetture backend e la gestione delle reti.

---

### 🚀 Progetti

#### 🌐 Web & Full-Stack Development

* **📚 [CozyShelf](https://cozyshelf.page)** `In Sviluppo Continuo`  
  Social network per amanti della lettura per scoprire libri, gestire scaffali virtuali e condividere consigli.  
  `TypeScript` • `React` • `Tailwind` • `Java (Spring Boot)` • `MongoDB` • `Cloudflare R2`

* **🏥 [SAW-MedHub](https://github.com/Giovy52845/SAW-MedHub)**  
  Piattaforma web ispirata a MioDottore per la gestione di appuntamenti e profili medici, con prenotazione slot, referti e notifiche in tempo reale.  
  `React (Vite)` • `Node.js` • `Express` • `Firebase` • `JavaScript`

---

#### 🛠️ Systems, Networking & Distributed Applications

* **📡 [AirWatch](https://github.com/Giovy52845/AirWatch)**  
  Tool in C per l'analisi del traffico Wi-Fi IEEE 802.11 in *monitor mode*. Analizza frame Beacon e Data in tempo reale o da file `.pcap` tramite `libpcap`.  
  `C` • `libpcap` • `Linux` • `IEEE 802.11` • `Bash / airmon-ng`

* **🕹️ [Connections-Java](https://github.com/Giovy52845/Connections)**  
  Sistema distribuito Client-Server Stateful basato sul gioco *Connections*. Socket TCP/UDP, Java NIO non bloccante, multi-threading e persistenza JSON.  
  `Java 21` • `Java NIO` • `Multi-threading` • `TCP/UDP Sockets` • `JSON / GSON`

* **🎲 [Paroliere-Online-C](https://github.com/Giovy52845/Paroliere-Online-C)**  
  Game Server distribuito in C per il gioco "Il Paroliere". Architettura multi-threaded con Pthreads, Socket POSIX, segnali Linux e strutture dati custom (Trie, Hash Table).  
  `C` • `POSIX Threads` • `POSIX Sockets` • `Trie` • `Valgrind`

---

### 🛠️ Competenze & Tecnologia

* **Sviluppo Web & Backend:** TypeScript, JavaScript, Java, Spring Boot, React, C
* **Reti & Infrastruttura:** Linux, Docker, Cloudflare R2, API REST
* **Database:** MongoDB, Firebase
* *Background accademico anche su Assembly e Verilog.*
